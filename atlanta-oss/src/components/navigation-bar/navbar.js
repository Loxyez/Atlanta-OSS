...
export default function CustomNavbar() {
  ...
  const navGroups = {
    ...
    staffManagement: {
      ...
      items: [
        ...
        {label: 'ปฏิทิน', icon: <CalendarMonth />, path: '/calendar'},
      ],
    },
    taskManagement: {
    ...
    requests: {
      ...
      items: [
        {
          label: 'ส่งคำร้องขอ ลากิจ/ลาอื่นๆ',
          icon: <RequestPage />,
          path: '/request_leave',
          disabled: !isLoggedIn,
        },
    ...
    guest: {
      ...
      items: [
        {
          label: 'ส่งคำร้องขอบัญชีเข้าใช้ระบบ',
          icon: <RequestPage />,
          path: '/request_form',
        },
  ...
  const renderDesktopMenu = (groupName, items, anchorEl, handleClose) => {
    ...
    return (
      <>
        <Button color='inherit' onClick={(e) => handleMenuOpen(e, groupName)} startIcon={getIcon()}>
          {getLabel()}
        </Button>
    ...
    <>
      <AppBar position='static'>
        <Toolbar>
          ...
          {!isMobile && isLoggedIn && (
            <>
              {getRoleBasedNavItems('stockManagement').length > 0 &&
                renderDesktopMenu(
                  'stockManagement',
                  getRoleBasedNavItems('stockManagement'),
                  stockManagementAnchorEl,
                  handleMenuClose
                )}
              {getRoleBasedNavItems('staffManagement').length > 0 &&
                renderDesktopMenu(
                  'staffManagement',
                  getRoleBasedNavItems('staffManagement'),
                  staffManagementAnchorEl,
                  handleMenuClose
                )}
              {getRoleBasedNavItems('taskManagement').length > 0 &&
                renderDesktopMenu(
                  'taskManagement',
                  getRoleBasedNavItems('taskManagement'),
                  taskManagementAnchorEl,
                  handleMenuClose
                )}
              {getRoleBasedNavItems('requests').length > 0 &&
                renderDesktopMenu('requests', getRoleBasedNavItems('requests'), requestsAnchorEl, handleMenuClose)}
              {getRoleBasedNavItems('account').length > 0 &&
                renderDesktopMenu('account', getRoleBasedNavItems('account'), accountAnchorEl, handleMenuClose)}
            </>
          )}
          {!isMobile && !isLoggedIn && (

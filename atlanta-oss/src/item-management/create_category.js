...
export default function CreateCategory() {
  ...
  const handleAddCategory = async () => {
    ...
    const isDuplicate = categories.some((category) => category.categoryname.toUpperCase() === uppercaseCategoryName);
  ...
  const handleSaveEdit = async (categoryid) => {
    ...
    try {
      await axios.put(
        `${config.apiBaseUrl}/categories/update_category/${categoryid}`,
        {
    ...
    <div>
      ...
      <Box sx={{p: 3}}>
        ...
        <Dialog open={confirmDelete} onClose={() => setConfirmDelete(false)}>
          ...
          <DialogContent>
            <DialogContentText>คุณแน่ใจหรือไม่ว่าต้องการลบหมวดหมู่นี้?</DialogContentText>
          </DialogContent>
          <DialogActions>
